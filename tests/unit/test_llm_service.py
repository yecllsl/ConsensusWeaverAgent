...
def test_analyze_question_invalid_json(llm_service):
    ...
    # 模拟无效JSON响应
    with patch.object(llm_service, 'generate_response', return_value='这不是有效的JSON格式'):
        result = llm_service.analyze_question("人工智能是什么？")
...
def test_analyze_question_with_chinese_commas(llm_service):
    ...
    with patch.object(llm_service, 'generate_response', return_value=json_with_chinese_commas):
        result = llm_service.analyze_question("人工智能是什么？")
...
def test_analyze_question_with_extra_text(llm_service):
    ...
    with patch.object(llm_service, 'generate_response', return_value=json_with_extra_text):
        result = llm_service.analyze_question("人工智能是什么？")
...
def test_refine_question_with_clarifications(llm_service):
    """测试重构问题时包含澄清信息的情况"""
    # 模拟有效的重构问题响应
    with patch.object(llm_service, 'generate_response', return_value='推荐三个适合开发命令行界面形式AI代理的开发框架，并详细说明每个框架的优缺点和适用场景'):
...
def test_refine_question_with_prefix(llm_service):
    """测试重构问题时LLM返回包含前缀的响应"""
    # 模拟包含前缀的重构问题响应
    with patch.object(llm_service, 'generate_response', return_value='重构后的问题：推荐三个适合开发命令行界面形式AI代理的开发框架，并详细说明每个框架的优缺点和适用场景'):
        result = llm_service.refine_question(
            "推荐三个开发cli形式AI Agent的开发框架，并说明他们的优缺点和是和场景",
            ["'是和场景'应为'适用场景'的笔误"]
        )
...
def test_refine_question_failure(llm_service):
    """测试重构问题失败的情况"""
    # 模拟重构失败
    with patch.object(llm_service, 'generate_response', side_effect=Exception("LLM生成响应失败")):
        original_question = "推荐三个开发cli形式AI Agent的开发框架，并说明他们的优缺点和是和场景"
        result = llm_service.refine_question(
            original_question,
            ["'是和场景'应为'适用场景'的笔误"]
        )
