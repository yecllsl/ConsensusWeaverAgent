...
class InteractionEngine:
    def __init__(self, llm_service: LLMService, data_manager: DataManager, max_clarification_rounds: int = 3):
    ...
    def generate_clarification(self, state: InteractionState, analysis: Dict[str, Any]) -> Optional[str]:
        """生成澄清问题"""
        ...
        # 检查是否需要澄清
        if analysis["is_complete"] and analysis["is_clear"] and not analysis["ambiguities"]:
        ...
        try:
            ...
            self.logger.info(f"生成澄清问题 ({state.clarification_rounds}/{self.max_clarification_rounds}): {clarification}")
    ...
    def handle_clarification_response(self, state: InteractionState, response: str) -> InteractionState:
        """处理用户的澄清响应"""
    ...
    def is_clarification_needed(self, analysis: Dict[str, Any]) -> bool:
        """判断是否需要澄清"""
        return not (analysis["is_complete"] and analysis["is_clear"] and not analysis["ambiguities"])
