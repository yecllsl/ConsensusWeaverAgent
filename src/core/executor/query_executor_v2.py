...
class QueryExecutorV2:
    ...
    async def execute_queries(
        ...
        self.logger.info(
            f"开始并行查询，会话ID: {session_id}, 使用工具: {tools}"
        )

        try:
            # 执行工具查询
            tool_results = await self.tool_manager.run_multiple_tools(
                question, tools
            )
            ...
            # 使用事务批量保存工具结果
            await self._save_tool_results_with_transaction(
    ...
    async def execute_single_query(
        ...
        """执行单个工具查询（使用事务管理）"""
        self.logger.info(
            f"开始单个查询，会话ID: {session_id}, 工具: {tool_name}"
        )
        ...
        try:
            ...
            # 使用事务保存工具结果
            await self._save_single_tool_result_with_transaction(
                session_id, result
            )
    ...
    async def get_query_results(
        ...
        try:
            async with self.transaction_manager.begin_transaction() as uow:
                # 从仓库获取工具结果
                entities = await uow.tool_results.get_by_session_id(
                    session_id
                )
    ...
    def validate_query_params(
        ...
        # 检查工具是否都可用
        enabled_tools = [
            tool.name for tool in self.tool_manager.enabled_tools
        ]
        for tool in tools:
