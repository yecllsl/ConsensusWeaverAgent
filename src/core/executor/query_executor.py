...
class QueryExecutor:
    ...
    async def execute_queries(self, session_id: int, question: str, tools: List[str]) -> QueryExecutionResult:
        """执行并行查询"""
        import time
        ...
        try:
            ...
            self.logger.info(f"并行查询完成，会话ID: {session_id}, 成功: {success_count}, 失败: {failure_count}, 总耗时: {total_execution_time:.2f}秒")
    ...
    async def execute_single_query(self, session_id: int, question: str, tool_name: str) -> ToolResult:
        """执行单个工具查询"""
        ...
        try:
            ...
            self.logger.info(f"单个查询完成，会话ID: {session_id}, 工具: {tool_name}, 成功: {result.success}")
    ...
    def validate_query_params(self, question: str, tools: List[str]) -> Tuple[bool, str]:
        """验证查询参数"""
