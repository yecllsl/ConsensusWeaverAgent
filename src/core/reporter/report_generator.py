...
class ReportGenerator:
    ...
    def _render_text_report(self, session: SessionRecord, tool_results: List[ToolResultRecord], analysis: AnalysisResultRecord) -> str:
        """渲染纯文本格式报告"""
        report_parts = []
        ...
        matrix_str = "  ".join([result.tool_name[:4] for result in tool_results if result.success])
        report_parts.append(f"      {matrix_str}")
        
        successful_results = [result for result in tool_results if result.success]
        for i, (row, result) in enumerate(zip(analysis.similarity_matrix, successful_results)):
    ...
    def export_report(self, session_id: int, format: str = "text", file_path: Optional[str] = None) -> str:
        """导出报告为指定格式"""
        try:
            if format != "text":
